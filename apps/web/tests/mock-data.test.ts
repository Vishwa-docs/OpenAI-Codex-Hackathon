...
describe("mock data fallback", () => {
  it("returns the seeded project snapshot when the API is unavailable", async () => {
    ...
    expect(project.intake.expectedUsers).toBeGreaterThan(0);
    expect(project.deploymentPlan.platformOptions.length).toBeGreaterThan(2);
  });
  ...
  it("keeps the seeded snapshot deterministic", () => {
    ...
    expect(project.observability.length).toBeGreaterThan(3);
    expect(project.mtcPipeline.some((stage) => stage.pipelineKey === "aws_execution")).toBe(true);
  });
});
